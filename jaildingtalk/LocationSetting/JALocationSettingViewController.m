...
@interface JALocationSettingViewController () <MKMapViewDelegate> {
...
- (void)viewDidLoad {
    ...
    _settingCoordinate = [JALocationHelper savedLocationCoordinate];
...
- (void)initViews {
    ...
    _annotationView.center = CGPointMake(self.view.ja_width / 2, self.view.ja_height / 2);
...
- (void)mapView:(MKMapView *)mapView regionDidChangeAnimated:(BOOL)animated {
    ...
    if (_mapView.alpha == 0) {
        [UIView animateWithDuration:0.15 animations:^{
            ...
            [_annotationView ja_stopLoading];
        }];
    }
}
...
- (void)search:(NSString *)keyword {
    ...
    [[[CLGeocoder alloc] init] geocodeAddressString:keyword completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
        if (error) {
            NSLog(@"Forward geocoding failed.");
...
- (void)locateCompany {
    CLLocationCoordinate2D coordinate = [JALocationHelper savedLocationCoordinate];
...
- (void)clearLocation {
    [JALocationHelper clearLocation];
...
- (void)commit {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        [JALocationHelper saveLocation:_settingCoordinate];
